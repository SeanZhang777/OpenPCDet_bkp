...
def box3d_overlap(boxes, qboxes, criterion=-1, z_axis=1, z_center=1.0):
    ...
    # rinc = rotate_iou_gpu_eval(boxes[:, bev_axes], qboxes[:, bev_axes], 1)
...
def get_split_parts(num, num_part):
    ...
    if remain_num == 0:
        ...
        return [remain_num] + [same_part] * num_part
...
def calculate_iou_partly(gt_annos,
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            gt_boxes = [a["gt_boxes_lidar"] for a in gt_annos_part]
            gt_boxes = np.concatenate(gt_boxes, axis=0)
            dt_boxes = [a["gt_boxes_lidar"] for a in dt_annos_part]
...
def get_mAP_eval_result(gt_annos,
                             dt_annos,
                             current_classes,
                             difficultys=[0],
                             z_axis=2,
    ...
    """
    overlap_mod = np.array([[[0.7, 0.7, 0.5, 0.5, 0.5]]])
    class_to_name = {
        0: 'Vehicle',
        1: 'Large_Vehicle',
        2: 'Pedestrian',
    ...
    min_overlaps = overlap_mod[:, :, current_classes]
    result = ''
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            result += print_str(
                (f"{class_to_name[curcls]} "
                 "AP(Average Precision)@{:.2f}:".format(*min_overlaps[i, :, j])))
...
def get_maxus_eval_result(gt_annos,
                          dt_annos,
                          current_classes,
                          difficultys=[0],
                          z_axis=2,
