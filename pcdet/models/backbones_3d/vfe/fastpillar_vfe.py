...
class MAPELayer(nn.Module):
    def __init__(self,
        ...
        self.max_encoding = nn.Sequential(
            nn.Linear(in_channels, out_channels, bias=False),
            nn.BatchNorm1d(out_channels, eps=1e-3, momentum=0.01),
            nn.LeakyReLU()
        )
        self.attn_encoding = nn.Sequential(
            nn.Linear(in_channels, out_channels, bias=False),
            nn.BatchNorm1d(out_channels, eps=1e-3, momentum=0.01),
            nn.LeakyReLU()
        )
    ...
    def forward(self, inputs, unq_inv):
        x_max_vfe = self.max_encoding(inputs)
        x_attn_vfe = self.attn_encoding(inputs)
        x_attn_score = torch_scatter.scatter_softmax(x_attn_vfe, unq_inv, dim=0)
        x_attn_vfe_mul = x_max_vfe * x_attn_score

        x_max = torch_scatter.scatter_max(x_max_vfe, unq_inv, dim=0)[0]
        x_attn = torch_scatter.scatter_sum(x_attn_vfe_mul, unq_inv, dim=0)

        return (x_max + x_attn) * 0.5

class FastPillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        ...
        for i in range(len(num_filters) - 1):
            ...
            pfn_layers.append(
                MAPELayer(in_filters, out_filters, self.use_norm, last_layer=(i >= len(num_filters) - 2))
            )
