...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, init_bias=-2.19, use_bias=False):
        ...
        for cur_name in self.sep_head_dict:
            ...
            if 'hm_center' in cur_name or 'hm_corner' in cur_name or 'hm' in cur_name or 'hm0' in cur_name or 'hm1' in cur_name or 'iou' in cur_name:
...
class FastPillarsHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
                 predict_boxes_when_training=False, val=False):
        super().__init__()
    ...
    def build_losses(self):
        ...
        if self.iou_branch:
            self.add_module('crit_iou', loss_utils.IouLoss())
            self.add_module('crit_iou_reg', loss_utils.IouRegLoss())
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            if self.iou_branch:
                batch_box_preds = self._get_predicted_boxes(pred_dict)
    ...
    def _get_predicted_boxes(self, pred_dict):
        center = pred_dict['center']
        center_z = pred_dict['center_z']
        rot = pred_dict['rot']
        #dim = pred_dict['dim'].exp()
        dim = torch.exp(torch.clamp(pred_dict['dim'], min=-6, max=6))
        # dim = dim.permute(0, 2, 3, 1).contiguous()
        batch, _, H, W = dim.size()
        rot_cos = rot[:, 0].unsqueeze(dim=1)
        rot_sin = rot[:, 1].unsqueeze(dim=1)
        angle = torch.atan2(rot_sin, rot_cos)
        ys, xs = torch.meshgrid([torch.arange(0, H), torch.arange(0, W)])
        ys = ys.view(1, 1, H, W).repeat(batch, 1, 1, 1).to(dim)
        xs = xs.view(1, 1, H, W).repeat(batch, 1, 1, 1).to(dim)
        xs = (xs + center[:, 0:1]) * self.feature_map_stride * self.voxel_size[0] + self.point_cloud_range[0]
        ys = (ys + center[:, 1:2]) * self.feature_map_stride * self.voxel_size[1] + self.point_cloud_range[1]
    ...
    def rotate_nms_iou(self, boxes, scores, iou_preds, labels, rectifier, nms_configs):
        """
        :param boxes: (N, 5) [x, y, z, l, w, h, theta]
        :param scores: (N)
        :param thresh:
        ...
        #     labels_list.append(labels_cls[selected])
        cls = 0
        scores_cls = torch.pow(scores, 1 - rectifier[cls]) * torch.pow(iou_preds.squeeze(-1), rectifier[cls])
        selected, selected_scores = model_nms_utils.class_agnostic_nms(box_scores=scores_cls, box_preds=boxes,
                                                    nms_config=nms_configs[cls], score_thresh=None)
        # return torch.cat(box_preds_list, dim=0), torch.cat(scores_list, dim=0), torch.cat(labels_list, dim=0)
        return boxes[selected], scores_cls[selected], labels[selected]
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        for k in range(batch_size):
            ...
            if self.iou_branch:
                ...
                pred_boxes, pred_scores, pred_labels = self.rotate_nms_iou(pred_boxes, pred_scores, pred_ious, pred_labels, self.rectifier, self.nms_configs)
    ...
    def forward(self, data_dict):
        ...
        data_dict['gt_hms'] = None
        data_dict['dt_hms'] = pred_dicts[0]['hm'].detach()
        data_dict['pred_dicts'] = pred_dicts
        data_dict['generate_predicted_boxes'] = self.generate_predicted_boxes
        if self.training:
            target_dict = self.assign_targets(
                data_dict['gt_boxes'], feature_map_size=spatial_features_2d.size()[2:]
            )
            self.forward_ret_dict['target_dicts'] = target_dict
            data_dict['gt_hms'] = target_dict['heatmaps'][0]
