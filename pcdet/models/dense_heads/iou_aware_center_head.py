...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, init_bias=-2.19, use_bias=False):
        ...
        for cur_name in self.sep_head_dict:
            ...
            if 'hm' in cur_name or 'keypoints' in cur_name:
...
class IouAwareCenterHead(nn.Module):
    ...
    def assign_target_of_single_head(
            ...
            feature_map_size, feature_map_stride, num_max_objs=500,
        ...
        # prepare corner
        corners = []
        rot = [torch.cos(yaw)[:, None], -torch.sin(yaw)[:, None], torch.sin(yaw)[:, None], torch.cos(yaw)[:, None]]
        rot = torch.cat(rot, dim=-1).reshape(-1, 2, 2)
        x_ind = [1, -1, -1, 1]
        y_ind = [1, 1, -1, -1]
        for i in range(4):
            temp_corner_x = x_ind[i] * 0.5 * dx
            temp_corner_y = y_ind[i] * 0.5 * dy
            corners.append(torch.cat((temp_corner_x[:, None], temp_corner_y[:, None]), dim=-1))
        corners = torch.stack(corners, dim=1)
        rot_corners = torch.matmul(rot, corners.transpose(1, 2)).transpose(1, 2)
        rot_corners_x = rot_corners[..., 0] + x[:, None]
        rot_corners_y = rot_corners[..., 1] + y[:, None]
        corner_coord_x = (rot_corners_x - self.point_cloud_range[0]) / self.voxel_size[0] / feature_map_stride
        corner_coord_y = (rot_corners_y - self.point_cloud_range[1]) / self.voxel_size[1] / feature_map_stride
        corner_coord_x = torch.clamp(corner_coord_x, min=0, max=feature_map_size[0] - 0.5)  # bugfixed: 1e-6 does not work for center.int()
        corner_coord_y = torch.clamp(corner_coord_y, min=0, max=feature_map_size[1] - 0.5)  #
        corners = torch.stack([corner_coord_x, corner_coord_y], dim=2)
        ...
        key_points_radius = radius / 2
        radius = torch.clamp_min(radius.int(), min=min_radius)
        key_points_radius = torch.clamp_min(key_points_radius.int(), min=min_radius / 2)
        ...
        iou_target = centernet_utils.get_iou_target(gt_boxes[:, :7], pred_dict, inds.cuda(), self.feature_map_stride,
                                                    self.voxel_size, self.point_cloud_range)
        ret_iou[:gt_boxes.shape[0]] = iou_target
        ret_iou = ret_iou.view(-1, 1)
        return heatmap, ret_boxes, inds, mask, keypoint_map, ret_iou

    def assign_targets(self, gt_boxes, feature_map_size=None, pred_dicts=None, **kwargs):
        """
        Args:
            gt_boxes: (B, M, 8)
        ...
        for head_idx, cur_class_names in enumerate(self.class_names_each_head):
            heatmap_list, target_boxes_list, inds_list, masks_list, \
                keypoint_map_list, target_iou_list = [], [], [], [], [], []
            for bs_idx in range(batch_size):
                cur_gt_boxes = gt_boxes[bs_idx]
                pred_dict = {}
                for k in pred_dicts[head_idx].keys():
                    pred_dict[k] = pred_dicts[head_idx][k][bs_idx].unsqueeze(0)
                ...
                heatmap, ret_boxes, inds, mask, keypoint_map, ret_iou = self.assign_target_of_single_head(
                ...
                keypoint_map_list.append(keypoint_map.to(gt_boxes_single_head.device))
                target_iou_list.append(ret_iou.to(gt_boxes_single_head.device))

            ret_dict['heatmaps'].append(torch.stack(heatmap_list, dim=0))
            ret_dict['target_boxes'].append(torch.stack(target_boxes_list, dim=0))
            ret_dict['inds'].append(torch.stack(inds_list, dim=0))
            ret_dict['masks'].append(torch.stack(masks_list, dim=0))
            ret_dict['keypoint_maps'].append(torch.stack(keypoint_map_list, dim=0))
            ret_dict['target_ious'].append(torch.stack(target_iou_list, dim=0))
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            pred_boxes = torch.cat([pred_dict[head_name] for head_name in self.separate_head_cfg.HEAD_ORDER
                                        if head_name not in ['keypoints', 'iou']], dim=1)
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        rescore_weight = post_process_cfg.RESCORE_WEIGHT
